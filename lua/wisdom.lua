...
--- @param max_width number
--- @return table
local format_wisdom = function(wisdom, max_width)
    -- Converts list of strings to one formatted string (with linebreaks)
    local formatted_wisdom = { " " } -- adds spacing between alpha-menu and footer
    for _, line in ipairs(wisdom) do
        local formatted_line = format_line(line, max_width)
        formatted_wisdom = list_extend(formatted_wisdom, formatted_line)
    end
    return formatted_wisdom
end

local get_wisdom = function(wisdom_list)
    -- selects an entry from wisdom_list randomly
    math.randomseed(os.time())
    local ind = math.random(1, #wisdom_list)
    return wisdom_list[ind]
end
...
local main = function(opts)
    ...
    local wisdom = get_wisdom(options.wisdom_list)
    local formatted_wisdom = format_wisdom(wisdom, options.max_width)
